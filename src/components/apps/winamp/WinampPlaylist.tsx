  ...
  return (
    <div style={playlistContainerStyle}>
      ...
      <div style={toolbarStyle}>
        <button style={toolbarBtnStyle} title="Add File">
          📄+
        </button>
        <button style={toolbarBtnStyle} title="Add Directory">
          📁+
        </button>
        <button style={toolbarBtnStyle} title="Add URL">
          🌐+
        </button>
        <div style={separatorStyle} />
        <button style={toolbarBtnStyle} title="Remove Selected">
          🗑️
        </button>
        <button style={toolbarBtnStyle} title="Clear Playlist">
          🧹
        </button>
        <div style={separatorStyle} />
        <button style={toolbarBtnStyle} title="Shuffle">
          🔀
        </button>
        <button style={toolbarBtnStyle} title="Repeat">
          🔁
        </button>
      </div>
          ...
          <div
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.backgroundColor =
                index === currentIndex ? '#316AC5' : '#E0E0E0';
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.backgroundColor =
                index === currentIndex ? '#316AC5' : 'transparent';
            }}
          >
            {/* Track Number */}
            ...
            {/* Track Time */}
            <div style={trackTimeStyle}>{formatTrackTime(index)}</div>
          </div>
        ))}
      </div>
      ...
      <div style={statusBarStyle}>
        <span>
          {tracks.length} files • Total time: {Math.floor(tracks.length * 3.5)}:30
        </span>
