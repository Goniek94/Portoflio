...
// 👇 IMPORT WINAMPA
import WinampWindow from '../Applications/Winamp/WinampWindow';
...
const DEFAULT_ICONS: IconData[] = [
  { id: 1, label: 'Gadu-Gadu',         src: '/icons/chat-icon.png',     x:  32, y:  32 },
  { id: 2, label: 'Counter-Strike 1.6',src: '/icons/game-icon.png',     x:  32, y: 140 },
  { id: 3, label: 'Internet Explorer', src: '/icons/winamp-icon.svg',   x:  32, y: 248 }, // 👈 UŻYJ DOSTĘPNEJ IKONY
  { id: 4, label: 'Kosz',              src: '/icons/trash-icon.png',    x:  32, y: 356 },

  { id: 5, label: 'Tibia',             src: '/icons/rpg-icon.png',      x: 150, y:  32 },
  { id: 6, label: 'Winamp',            src: '/icons/winamp-icon.svg',   x: 150, y: 140 }, // 👈 UŻYJ DOSTĘPNEJ IKONY
  { id: 7, label: 'GTA: San Andreas',  src: '/icons/action-icon.png',   x: 150, y: 248 },
  { id: 8, label: 'Need for Speed',    src: '/icons/racing-icon.png',   x: 150, y: 356 },
  { id: 9, label: 'Icy Tower',         src: '/icons/icy-icon.png',      x: 268, y:  32 }
];

const PORTFOLIO_ICONS: IconData[] = [
  { id: 101, label: 'CV.exe',       src: '/icons/document-icon.png', x: 386, y:  32 },
  { id: 102, label: 'Projekty.sys', src: '/icons/folder-icon.png',   x: 386, y: 140 },
  { id: 103, label: 'O_mnie.dll',   src: '/icons/profile-icon.png',  x: 386, y: 248 }
];
...
export default function DesktopIcons({ 
    ...
    setIcons(prev =>
      prev.map(icon =>
        icon.id === draggingIconId
          ? { ...icon, x: clientX - offsetX, y: clientY - offsetY }
          : icon
      )
    );
  }
  ...
  function sortIcons() {
    setIcons(prev => {
      ...
      const cols = 4, colW = 118, rowH = 108, startX = 32, startY = 32;
  ...
  return (
    ...
    >
        ...
        >
          ...
          <div
            style={{
              ...
              lineHeight: 1.2, wordWrap: 'break-word', maxWidth: 64
            }}
          >
            {icon.label}
          </div>
      ...
      {/* ----------  OTWARTE APLIKACJE  ---------- */}
      {openApps.includes('winamp') && (
        <WinampWindow onClose={() => handleCloseApp('winamp')} />
      )}
